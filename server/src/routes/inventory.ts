...
router.post('/', requireAdmin, async (req, res) => {
  try {
    ...
    // If a coverArtUrl is provided, validate it, it can be optional.
    if (coverArtUrl && coverArtUrl.trim() !== '' && !isValidUrl(coverArtUrl.trim())) {
...
router.patch('/:id', requireAdmin, async (req, res) => {
  try {
    ...
    if (coverArtUrl !== undefined && coverArtUrl.trim() !== '' && !isValidUrl(coverArtUrl.trim())) {
      return res.status(400).json({ error: 'Invalid cover art URL' });
    }
