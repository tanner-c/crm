...
router.get('/', requireAdmin, async (_req, res) => {
    try {
        const activities = await prisma.activity.findMany({
            ...
            include: { owner: true, customer: true, sale: true },
        });
...
router.post('/', requireAuth, async (req, res) => {
    try {
        const { type, subject, body, dueAt, completed, ownerId, customerId, saleId } = req.body;
        ...
        if (saleId !== undefined) data.saleId = saleId;
